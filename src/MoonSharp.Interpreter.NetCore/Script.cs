...
public class Script : IScriptPrivateResource
{
    ...
    static Script()
	{
		...
		DefaultOptions = new ScriptOptions()
		{
			...
			DebugInput = Script.GlobalOptions.Platform.DefaultInput,
    ...
    private (DynValue Result, SourceCode? Code) LoadStringWithReturnSourceCode(ReadOnlyMemory<char> code, Table? globalTable = null, string? codeFriendlyName = null)
    {
		...
		return (MakeClosure(address, globalTable ?? m_GlobalTable), source);
    }
...
#pragma warning restore 618
    ...
    public DynValue DoStringAndRemoveSource(ReadOnlyMemory<char> code, Table? globalContext = null, string? codeFriendlyName = null)
    {
		...
		if(codeSource != null && m_Sources[^1] == codeSource)
		{
			m_Sources.RemoveAt(m_Sources.Count - 1);
		}
	...
	public DynValue Call(DynValue function, params DynValue[] args)
	{
		...
		if (!function.IsFunction && !function.IsClrFunction)
		{
		...
		else if (function.IsClrFunction)
		{
